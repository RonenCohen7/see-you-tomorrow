...
export async function getOrgSettings(req: AuthRequest, res: Response) {
  ...
  const managerCanEditSchedules = await orgSettings.getManagerCanEditSchedules();
  res.json({ managerCanEditSchedules });
}

export async function patchOrgSettings(req: AuthRequest, res: Response) {
  const value = Boolean(req.body.managerCanEditSchedules);
  await orgSettings.setManagerCanEditSchedules(value);
  res.json({ managerCanEditSchedules: value });
}
...
export async function month(req: AuthRequest, res: Response) {
  ...
  const dayMap = new Map<string, { office: number; home: number; vacation: number; sick: number; off: number }>();
  for (const row of filtered) {
    const k = row.workDate;
    const cur = dayMap.get(k) ?? { office: 0, home: 0, vacation: 0, sick: 0, off: 0 };
    cur[row.status as keyof typeof cur]++;
    dayMap.set(k, cur);
  }
